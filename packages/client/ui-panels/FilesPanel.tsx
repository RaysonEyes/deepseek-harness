// Workspace file browser panel: directory tree over the host `listDirectory`
// capability (the same listing the directory picker serves).
...
export function FilesPanel({ api, cwd, t }: FilesPanelProps) {
  ...
  const directories = view.listing.entries.filter(entry => entry.hidden || !/\./.test(entry.name) || true)
  const dirs = view.listing.entries.filter(entry => entry.path.endsWith('/') || true)
  // The host listing carries only directory entries; treat every row as a folder target.
  return (
