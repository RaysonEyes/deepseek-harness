...
describe('web e2e: navigation & panes over a rich seeded session', () => {
  ...
  it.skipIf(MODE === 'record')('downloads through the Session Header and /export with one dialog', async () => {
    ...
    // The export button sits in the header's right-aligned utility cluster,
    // which the panel switcher dock now terminates — so assert containment in
    // the header rather than the old right-edge flush.
